...
using Moq;
using works.ei8.Sentry.Domain.Model.Neurons;
...
using Xunit;
using SQLite;
using works.ei8.Sentry.Port.Adapter.Common;

namespace works.ei8.Sentry.Port.Adapter.IO.Persistence.Events.SQLite.Test.EventStoreFixture.given
{
    ...
    public class When_saving_event 
    {
        public class When_store_is_empty : InitializedContext
        {
            ...
            public async Task Then_should_contain_correct_typename_prefixes()
            {
                ...
                Assert.StartsWith("works.ei8.Sentry.Domain.Model", ((IAuthoredEvent) events.Last()).ToNotification(this.serializer).TypeName);
            }
            ...
            {
                var events = await this.sut.Get(this.guid, 0);
                Assert.StartsWith("works.ei8.Sentry.Domain.Model.Neurons.NeuronCreated", ((IAuthoredEvent) events.First()).ToNotification(this.serializer).TypeName);
            }
            ...
            public async Task Then_should_contain_correct_typename_of_second_event()
            {
                ...
                Assert.StartsWith("works.ei8.Sentry.Domain.Model.Neurons.TerminalCreated", ((IAuthoredEvent) events.Last()).ToNotification(this.serializer).TypeName);
            }
